...
PREPCOMP_CXX=proc code=cpp cpp_suffix=cpp maxopencursors=50 MODE=ANSI TYPE_CODE=ANSI CLOSE_ON_COMMIT=yes def_sqlcode=yes dynamic=ANSI define=ESQL_ORA LINES=YES  include="/usr/include"  include="$(ORACLE_HOME)/precomp/public" include="$(ORACLE_HOME)/rdbms/public" include="$(HOME)/ykt/src/buinc" include="$(HOME)/ykt/src/include" include="$(HOME)/ykt/src/include/dbincl" include="$(HOME)/ykt/src/public" $(PROC_OPTS)
...
ifeq ($(UNAME),Linux)
	...
	CCFLAGS= -D_DRTP_V4 -D_MT -m32 -Wall  -DLINUX -O3 -Wall -Wreturn-type -Wuninitialized -Wfloat-equal -Wredundant-decls $(USEROPTS)
	CXXFLAGS= -D_DRTP_V4 -D_MT -m32 -Wall  -DLINUX -O3 -Wall -Wreturn-type -Wuninitialized -Wfloat-equal -Wredundant-decls $(USEROPTS)
	LIB_SUFFIX=
endif
ifeq ($(UNAME),AIX)
	CC=gcc
