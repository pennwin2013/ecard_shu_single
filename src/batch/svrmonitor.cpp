...
int ini_getstr(const char *i_inifile, const char *i_key, char *o_value)
{
    FILE *fp_ini = NULL;
...
void daemon1(const char* conf)
{
    ...
    sprintf(inifile, "%s/%s.ini", p, conf);
    for(i = 0; i < MAX_PROC;)
    {
        sprintf(key, "CMD%d", i);
    ...
    int const proc_num = i;
    while(1)
    {
        sleep(30);
        for(i = 0; PL[i].pid != 0 && i < proc_num; i++)
        {
            if(PsPid(PL[i].pid) != 0)
            {
                fprintf(stdout, "Restarting %s\n", PL[i].proc_cmd);
                fflush(stdout);
                PL[i].pid = init_daemon(PL[i].proc_cmd);
            }
        }
...
{
    char  szVerNo[61] = {0};
    sprintf(szVerNo, "svrmonitor V4.0(%s %s)", __DATE__, __TIME__);
    const char short_opts[] = "vV";
    int option;
