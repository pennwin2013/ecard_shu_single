...
#define	TRANS_INI_FILE 			"./trans.ini"		//ini文件
  ...
  typedef struct
  {
    ...
    char		TradeDate[9];		//sdate0			//交易日期	 YYYYMMDD
    char		TradeTime[7];		//stime0			//交易时间	 HHMMSS
    char		BankDate[9];			//sdate1			//银行处理日期
    char		BankTime[7];			//stime1			//银行处理时间
    ...
    int		  MngFeeFlag;			//sstatus1		//手续费标志
    char		AccountPwd[6];		//semp_pwd		//校园账户密码
    char		AccountNewPwd[6];	//semp_pwd2		//校园账户新密码
    char		BankCardPwd[17];		//sstation0		//银行账户密码
    char		BankCardNewPwd[17];	//sstation1		//银行账户新密码
